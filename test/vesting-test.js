...
describe("Vesting", function () {
  ...
  it("Should delete a user and funds after some vesting stuff", async function () {
    ...
    expect(balance).to.eq(aa-i);

    // check things are in order
    expect(await token.balanceOf(ana.address)).to.eq(i);             // only one transfer
    expect(await token.balanceOf(owner.address)).to.eq(tokens-aa);   // aa tokens allocated to the contract
    expect(await token.balanceOf(vault.address)).to.eq(aa - i);      // contract got some tokens - minus the ones it spent - 40,000,000
    ...
    expect(await token.balanceOf(ana.address)).to.eq(i);                  // see above
    expect(await token.balanceOf(owner.address)).to.eq(tokens-(i));       // because we have allocated 40,000,000 already
    expect(await token.balanceOf(vault.address)).to.eq(0);
  });
  ...
  it("Should close and return funds to owner", async function () {
    ...
    // should fail
    await expect(vault.transfer([0], 0)).to.be.revertedWith("not-active");
